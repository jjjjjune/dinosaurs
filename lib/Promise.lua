--[[
	...
	Fork of module found in Roblox core scripts. This fork includes:
		* Promise.all creates a copy of the promises passed into it.
]]
...
function Promise.all(...)
	...
	if type(promises[1]) == "table" and promises[1]._type ~= "Promise" then
		...
		local copy = {}
		for i, v in ipairs(promises[1]) do
			copy[i] = v
		end
		promises = copy
	end
...
function Promise:_reject(...)
	...
	if not isEmpty(self._queuedReject) then
		...
		local message = ("Unhandled promise rejection:\n\n%s\n\n%s"):format(
			TableUtilities.RecursiveToString((...)),
			self._source
		)
		warn(message)
	end
